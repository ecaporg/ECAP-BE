...
async function fetchStudentForCourse(courseId, page = 1) {
    try {
        const response = await fetch(`https://eliteaa.instructure.com/api/v1/courses/${courseId}/users?per_page=200&page=${page}`, {
            headers: {
...
async function fetchAllCourses() {
    ...
    for (const course of allCourses) {
        ...
        if (course.all_users.length <
            course.total_students + course.teachers.length) {
            for (let page = 2; page < 2; page++) {
                course.all_users.push(...(await fetchStudentForCourse(course.id, page)));
            }
        }
...
fetchAllCourses();
//# sourceMappingURL=courses-students.js.map