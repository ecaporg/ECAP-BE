    ...
    try {
        const response = await fetch('https://mountainelite.plsis.com/mod.php/admin/registration/studentlist.php?action[getEntityListPageJson]=1', {
            headers: {
                ...
                Referer: 'https://mountainelite.plsis.com/mod.php/admin/registration/studentlist.php?action[Prompt][list]=1&&__sp_window_id__=1821518988',
...
async function fetchSISTeacher(fromFile = false) {
    ...
    try {
        const response = await fetch('https://mountainelite.plsis.com/mod.php/admin/staff_list.php?action[getEntityListPageJson]=1', {
            headers: {
                accept: 'text/javascript',
                'accept-language': 'uk-UA,uk;q=0.9,en-US;q=0.8,en;q=0.7',
                'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
                ...
                'x-requested-with': 'XMLHttpRequest',
                cookie: 'PHPSESSID=EDlatiGBjfIOnDei8Tw4_Q',
                Referer: 'https://mountainelite.plsis.com/mod.php/admin/staff_list.php?action[prompt]%5Blist%5D=1&&__sp_window_id__=1821518988',
                'Referrer-Policy': 'strict-origin-when-cross-origin',
            },
...
const mergeStudentsAndSIS = (students, sis, key = 'entity_district_id') => {
    return students.map((student) => {
        const sisStudent = sis.find((s) => s[key].toString() === student.sis_user_id);
...
async function fetchAll() {
    ...
    const teachersWithSIS = mergeStudentsAndSIS(teachers, sisTeachers, 'entity_legacy_id');
    ...
    writeFileSync('admins.json', JSON.stringify([
        {
