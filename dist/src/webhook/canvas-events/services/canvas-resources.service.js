...
let CanvasResourcesService = class CanvasResourcesService {
    ...
    async fetchCourse(key, courseId) {
    ...
    async fetchAssignmentsInCourse(key, courseId) {
        ...
        return (0, rxjs_1.firstValueFrom)(this.httpService
            ...
            .pipe((0, rxjs_1.map)((response) => response.data), (0, rxjs_1.map)((data) => data.flatMap((item) => item.assignments))));
    }
    ...
    fetchStudentsInCourse(key, courseId) {
        const url = `${key.url}/api/v1/courses/${courseId}/users?include%5B%5D=last_login&include%5B%5D=avatar_url&include%5B%5D=pronouns&include%5B%5D=email&include%5B%5D=time_zone&include%5B%5D=ui_invoked`;
        const params = {
